...
const els={},storage={};const el=id=>els[id]??={value:'',innerHTML:'',textContent:'',classList:{toggle(){}}};
const ctx=vm.createContext({document:{getElementById:el,addEventListener(){}},localStorage:{getItem:k=>storage[k],setItem:(k,v)=>storage[k]=v},console});
...
console.log('PASS: '+n+' assertions; five periods, three artifacts, progression, adaptation, savings, budget deviations and one-time rewards.');
