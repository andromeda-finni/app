...
http.Response _jsonResponse(Object body, int statusCode) =>
    http.Response(jsonEncode(body), statusCode, headers: {'content-type': 'application/json; charset=utf-8'});
...
void main() {
  testWidgets('no saved token -> shows onboarding', (tester) async {
    ...
    await tester.pumpWidget(GroshikApp(
      authStorage: authStorage,
      apiClient: ApiClient(httpClient: client, authStorage: authStorage, baseUrl: 'http://test'),
    ));
  ...
  testWidgets('saved token + GET /pet 200 -> shows the home placeholder, not onboarding', (tester) async {
  ...
  testWidgets('saved token + GET /pet 404 -> resumes onboarding without re-registering', (tester) async {
    final authStorage = FakeAuthStorage(initialToken: 'tok');
    var registerCalls = 0;
    final client = MockClient((request) async {
