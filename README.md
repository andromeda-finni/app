...
Для локального Docker `APP_DATABASE_URL` должен содержать `sslmode=disable`: PostgreSQL доступен только через loopback и TLS в контейнере не настроен. Для staging/production разрешён только `sslmode=verify-full` с CA-сертификатом провайдера. Режим `sslmode=prefer` намеренно не используется: его семантика различается между `libpq` и Node.js-драйвером `pg`.
