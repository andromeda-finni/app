...
class _MoleGameScreenState extends State<MoleGameScreen> {
  ...
  Future<void> _startServerQuest() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _syncNote = error.statusCode == 409
            ? 'Это задание уже начато или пройдено. Сейчас доступен тренировочный повтор.'
            : 'Нет связи с сервером. Играть можно, но награда в тренировке не начисляется.';
      });
    } finally {
  ...
  Future<void> _continueAfterEpisode() async {
    ...
    if (_assignmentId != null) {
      ...
      try {
        ...
        if (!mounted) return;
        setState(() {
          _syncNote = 'Не получилось сохранить результат. Проверь соединение и повтори.';
        });
        return;
      } finally {
