...
class _TugrikiGameScreenState extends State<TugrikiGameScreen>
  ...
  final Set<String> _selectedGoodIds = {
    'soup',
    'juice',
  }; // по умолчанию Суп (8м) + Сок (4м) = 12м
  String? _assignmentId;

  // Everything the finale says about money comes from the server's answer:
  // the amount actually credited, or the reason it was not.
  int? _rewardAmount;
  String? _rewardNote;
  ...
  Future<void> _completeServerQuest() async {
    if (widget.apiClient == null) {
      setState(
        () => _rewardNote = 'Тренировочный режим: награда не начисляется.',
      );
      return;
    }
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              children: [
                ...
                Expanded(flex: 34, child: _buildRomanceClubDialogArea()),
              ],
            ),
          ),
        ),
  ...
  Widget _buildRomanceClubScene() {
    ...
    final bool isSparrowSpeaking =
        _step == NovelStep.s1SparrowIntro ||
        _step == NovelStep.s4BunOrder ||
        _step == NovelStep.s6BunEnjoy ||
        _step == NovelStep.s7LunchMenu ||
        _step == NovelStep.s9LunchSuccess ||
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final charHeight =
            height * 0.72; // Высота персонажей пропорциональна сцене
    ...
    switch (_step) {
      ...
      case NovelStep.s1SparrowIntro:
        return _buildRomanceSpeechCard(
          ...
          isSparrow: true,
          dialogueText:
              '${widget.petName}, посмотри вокруг! Мы на ярмарке в соседнем государстве. Но здесь на всех прилавках цены в тугриках, а у меня с собой только монетки!',
      ...
      case NovelStep.s2GroshikEnters:
        return _buildRomanceSpeechCard(
          speakerName: widget.petName,
          isSparrow: false,
          dialogueText: 'Не переживай, друг! В другой стране свои деньги, но их легко пересчитать. Смотри на курс: за 1 тугрик нужно отдать 2 монетки!',
      ...
      case NovelStep.s3ExchangeRateDemo:
        return _buildRomanceSpeechCard(
          speakerName: 'Друг Воробей',
          isSparrow: true,
          dialogueText: 'Ага! Значит, 1 тугрик = 2 монетки. Давай скорее пойдём к булочнику, я ужасно проголодался!',
      ...
      case NovelStep.s4BunOrder:
        return _buildRomanceSpeechCard(
          speakerName: 'Друг Воробей',
          isSparrow: true,
          dialogueText: 'Булочка стоит 6 тугриков. Помоги мне отсчитать монетки! Нам нужно положить по 2 монетки на каждый тугрик.',
      ...
      case NovelStep.s6BunEnjoy:
        return _buildRomanceSpeechCard(
          speakerName: 'Друг Воробей',
          isSparrow: true,
          dialogueText: 'Ура-а-а! 12 монеток превратились в 6 тугриков, и тёплая булочка у меня в руках! Это было так понятно!',
      ...
      case NovelStep.s7LunchMenu:
        return _buildRomanceSpeechCard(
          speakerName: 'Друг Воробей',
          isSparrow: true,
          dialogueText: 'Давай закажем обед: Суп (4) + Сок (2) + Булочка (3). Всего 9 тугриков! Сколько монеток нужно обменять по курсу 1:2?',
      ...
      case NovelStep.s9LunchSuccess:
        return _buildRomanceSpeechCard(
          speakerName: 'Друг Воробей',
          isSparrow: true,
          dialogueText: 'Вкуснотища! 9 тугриков умножили на 2 — получилось ровно 18 монеток. Обед куплен!',
      ...
      case NovelStep.s10BudgetIntro:
        return _buildRomanceSpeechCard(
          speakerName: widget.petName,
          isSparrow: false,
          dialogueText: 'У нас в кошельке осталось ровно 20 монеток. Выбирай покупки с умом, чтобы хватило денег и осталась сдача!',
  ...
  Widget _buildRomanceSpeechCard({
    ...
    return Column(
      ...
      children: [
        ...
        StoryButton(label: actionText, showFlourish: true, onPressed: onAction),
      ],
    );
  }
        ...
        children: [
          ...
          Wrap(
            ...
            children: _budgetGoods.map((good) {
              ...
              return FilterChip(
                label: Text(
                  '${good.iconEmoji} ${good.name} (${good.priceTugriki}т = ${good.priceCoins}м)',
                ),
                selected: isSel,
