...
class QuestMapScreen extends StatefulWidget {
  ...
  /// Temporary local progress for the visual prototype. The screen is already
  /// structured so this value can later come from persisted game progress.
...
class _QuestMapScreenState extends State<QuestMapScreen>
  ...
  void _scrollToCurrent() {
    ...
    final target = (nodeY - viewport * 0.48).clamp(
      0.0,
              ...
              if (canPlay)
                ...
                OutlinedButton(
                  ...
                  child: const Text(
                    ...
                    style: TextStyle(fontFamily: AppFonts.family, fontSize: 15),
                  ),
                ),
            ],
          ),
    ...
    return Stack(
      ...
      children: [
        ...
        for (var i = 0; i < questMapNodes.length; i++)
          _positionNode(
            i,
            cloudFade: i == unlockedIndex ? (revealComplete ? 0.0 : 1 - revealValue) : null,
          ),
        Positioned(
          ...
          child: IgnorePointer(
            child: Semantics(
              label: 'Грошик идёт к заданию ${questMapNodes[unlockedIndex].title}',
              image: true,
              child: Container(
            ...
            Positioned(
              ...
              child: IgnorePointer(
                ...
                child: Opacity(
                  ...
                  child: _CloudCover(showLock: state == QuestMapNodeState.locked),
                ),
              ),
            ),
          ],
      ...
      children: [
        ...
        if (showLock)
          Container(
            ...
            child: const Icon(Icons.lock_rounded, color: Colors.white, size: 21),
          ),
      ],
    );
  }
  ...
  void _paintRoute(Canvas canvas) {
    ...
    if (unlockedIndex > 0) {
      ...
      for (var i = 1; i <= unlockedIndex; i++) {
        ...
        final next = nodes[i];
        final middleY = (previous.dy + next.dy) / 2;
        opened.cubicTo(previous.dx, middleY, next.dx, middleY, next.dx, next.dy);
      }
      canvas.drawPath(
        opened,
        Paint()
